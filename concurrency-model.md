...
There are two different ways we could perform locking on the Kerf Tree. I haven't tested which one is better but I suspect it's Option 2. It may actually depend on the usage model. Basically, the tradeoff is between maintaining paths through the tree versus not maintaining paths but claiming subtrees. If the paths are long or contentended it may make sense to use subtrees. If the subtrees are big it may be better to use paths. Most accesses should take place at leaves, which means "subtrees" are of size 1 and don't extend past the node itself. They only reason accesses wouldn't take place at leaves is if you were performing introspection into "directories" of the tree, such as copying libraries or some such, and this is odd behavior.
...
Only the nested maps that can be dot separated subsequences of strings that indicate globals need to be upgraded when placed on the tree. A map with a key of Int(1) or a general object does not need. Of course, it depends whether you want such maps to be considered first-class members of the subtree, which may be desirable. This is of limited value, it might be cleaner to convert everything.
